...
Naivebayes.prototype.categorize = function(text, verbose = false){
  ...
  categories.forEach(category => {
    ...
    if (verbose){
      ...
      likelihoods.push({ "label" : category, logLikelihood, logLikelihood_idf });
  ...
  if (verbose) {
    ...
    likelihoods.forEach((category) => {
      ...
      category.scaledLikelihood = Math.round(deltaValue * 100 / (maxLikelihood - minLikelihood), 10);
    });

    return {
      likelihoods,
      "predictedLabel": predictedCategory
    };
  }else{
